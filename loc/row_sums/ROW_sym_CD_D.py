...
# returns rowsums before and after row-normalization (with density normalization)
...
def ROW_sym_CD_D(data1,nsig):
    ...
    d1 = w1.sum(axis=1)
    da = d1.reshape((n1,1), order="F")
    db = d1.reshape((1,n1), order="F")
    q = np.multiply(da,db)
    w1 = np.divide(w1,q)
    w_sum = w1.sum(axis=1)
